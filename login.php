...
$sql = "SELECT id, email, 'cliente' as tipo, NULL as rol_id FROM clientes WHERE email = '$email' AND clave = '$password'
    ...
    echo "<script>alert('Inicio de sesión exitoso. Tipo de usuario: {$_SESSION['tipo_usuario']}, ID de usuario: {$_SESSION['usuario_id']}'); window.location.href = 'gestionar-mi-perfil.php';</script>";
} else {
